...
def _sleep_until(dt):
    """Sleep until the given datetime (replacement for the pause package)."""
    remaining = (dt - datetime.now(tz=dt.tzinfo)).total_seconds()
    if remaining > 0:
...
def run(driver, shoe_type, username, password, url, shoe_size, shipping_option, login_time=None, release_time=None,
    ...
    if login_time:
        ...
        _sleep_until(date_parser.parse(login_time))
    ...
    if release_time:
        ...
        _sleep_until(date_parser.parse(release_time))
